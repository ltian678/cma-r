...
parser.add_argument(
		"-train_dir",
		type=str,
		default="/train_data/story_twotier_input_train_v2.pkl"
)



parser.add_argument(
		"-test_dir", default="/train_data/story_twotier_input_test_v2.pkl", type=str
)
...
parser.add_argument(
	"-pretrained_model_path", default='res/models/', type=str
)
...
#Two-tier intervention should only be applied on the first layer transformer!
#The second tier transformer parameters should not be updated 

def train(rank, num_gpus, train_dir, test_dir, num_class, pretrained, pretrained_model_path, version_number):
	...
	train_data_loader = DataLoader(train_dataset, batch_size=train_batch_size, sampler=train_sampler, collate_fn=collate_fn,num_workers=num_workers)
	validation_data_loader = DataLoader(validation_dataset, batch_size= train_batch_size, sampler=validation_sampler, collate_fn=collate_fn,num_workers=num_workers)
	test_data_loader = DataLoader(test_dataset, batch_size=test_batch_size, sampler = test_sampler, collate_fn=collate_fn,num_workers=num_workers)
	...
	print('len of dataloader ', len(train_data_loader))
	...
	for epoch_i in range(0, epochs):
			...
			# Synchronize and collect validation loss and accuracy across all processes
			torch.distributed.barrier()
			total_eval_loss = torch.tensor(total_eval_loss).to(device)
			total_eval_accuracy = torch.tensor(total_eval_accuracy).to(device)
			torch.distributed.all_reduce(total_eval_loss)
			torch.distributed.all_reduce(total_eval_accuracy)

			# Report the final accuracy for this validation run.
			avg_val_loss = total_eval_loss.item() / len(validation_data_loader)
			avg_val_accuracy = total_eval_accuracy.item() / len(validation_data_loader)
	...
	for batch in test_data_loader:
		...
		with torch.no_grad():
			# Forward pass, calculate logit predictions
			outputs = model(story_input_ids=batch['input_data']['story_input_ids'], 
										story_attention_mask=batch['input_data']['story_attention_mask'], 
										lst_input_ids=batch['input_data']['lst'], 
										lst_attention_masks=batch['input_data']['lst_attention_masks'])
		logits = outputs[0]
	...
	all_predictions = torch.cat([torch.tensor(predictions[i]) for i in range(torch.distributed.get_world_size())], dim=0).numpy()
	all_true_labels = torch.cat([torch.tensor(true_labels[i]) for i in range(torch.distributed.get_world_size())], dim=0).numpy()

	# Calculate and print the final accuracy
	flat_predictions = np.argmax(all_predictions, axis=1).flatten()
	...
	print('Im into the saving process!!!')
...
def run(rank, num_gpus):
	...
	# Create your model and move it to the current GPU
	#model = YourModel()
	#model = model.to(device)
	...
	train(rank, num_gpus, opt.train_dir, opt.test_dir, opt.num_class, opt.pretrained, opt.pretrained_model_path, opt.version_number)
...
if __name__ == "__main__":
		...
		print('Here is num_gpus ', num_gpus)
		#num_gpus = torch.cuda.device_count()
