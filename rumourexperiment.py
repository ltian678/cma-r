...
class Model():
	...
	def get_representations(self, context, position):
		...
		def extract_representation_hook(module,
			...
			representations[layer] = output[0]
		handles = []
		representation = {}
		with torch.no_grad():
	...
	def get_probabilities_for_examples(self, context, candidates):
		...
		#What the function should do here is t
		#for c in candidates:
		#    if len(c) > 1:
		...
		#outputs_test = [c[0] for c in candidates] 
		#print('og outputs_test', outputs_test)
		outputs = candidates
		...
		print('return probs[:, outputs].tolist() ,',probs[:, outputs].tolist())
		return probs[:, outputs].tolist()
	
	def get_probabilities_for_rumours(self, input_ids, attention_masks):
	  if attention_masks == None:
		attention_masks = torch.tensor([1] * len(input_ids)).unsqueeze(0).to(self.device)
		print('len of input_ids', len(input_ids))
		print('im here to gen attention mask ', attention_masks.shape)
			...
			if layer == -1:
				...
				handle_list.append(self.neuron_layer(layer).register_forward_hook(
					partial(intervention_hook,
							...
							intervention=intervention_rep,
							intervention_type=intervention_type)))
		print('INTO NEW PROB ', len(context))
	...
	def neuron_intervention_experiment(self, comments2intervention, intervention_type, layers_to_adj=[], neurons_to_adj=[], alpha=1, intervention_loc='layer'):
		'''
		...
		for comm in tqdm(comments2intervention, desc='comments'):
			print('comm ', comm)
			comments2intervention_results[comm] = self.neuron_intervention_single_experiment(
	...
	def neuron_intervention_single_experiment(self,
		...
		with torch.no_grad():
			...
			if self.is_bert or self.is_distilbert or self.is_roberta or self.is_xlnet:
				num_alts = len(intervention.candidate_comments)
				#remove the masks as we have attention masks when we build up the intervention
				#masks = torch.tensor([self.st_ids[0]]).repeat(num_alts, 1).to(self.device)
				#intervention.og_tok = torch.cat(
				#    (intervention.og_tok, masks), dim=1)
			base_representations = self.get_representations(
				intervention.og_input_ids,
				intervention.position_lst[1])
			modified_representations = self.get_representations(
				intervention.candidate_combo_tok_input_ids[0],
				intervention.position_lst[1])


			print('base_representations ', type(base_representations))
			print('base_representations ', base_representations[0].shape)


			candidate1_base_prob, candidate2_base_prob = self.get_probabilities_for_rumours(
			  intervention.og_input_ids.unsqueeze(0),
			  intervention.og_attention_masks.unsqueeze(0)
			)[0]
			#candidate1_base_prob, candidate2_base_prob = self.get_probabilities_for_examples(
			#    intervention.og_tok.unsqueeze(0),
			#    intervention.padded_input[0].unsqueeze(0))[0]
			candidate1_alt1_prob, candidate2_alt1_prob = self.get_probabilities_for_rumours(
				intervention.candidate_combo_tok_input_ids[0].unsqueeze(0),
				intervention.candidate_combo_tok_attention_masks[0].unsqueeze(0))[0]
			candidate1_alt2_prob, candidate2_alt2_prob = self.get_probabilities_for_rumours(
				intervention.candidate_combo_tok_input_ids[1].unsqueeze(0),
				intervention.candidate_combo_tok_attention_masks[1].unsqueeze(0))[0]
			...
			if intervention_loc == 'all':
			  ...
			  print('Im HERE WITH ALL')
			  print('candidate1_probs ', candidate1_probs.shape)
			  ...
			  candidate1_probs = 0
			  candidate2_probs = 0
			  #TODO: need to init these two properly
			  probs = self.neuron_intervention(
						context=context,
						outputs=intervention.label_tok,
						rep=rep,
